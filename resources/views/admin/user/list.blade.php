        ...
        window.onload = () => {
            const Columns = [
                    ...
                    },
                    {
                        ...
                        clickToSelect: false,
                        formatter : function ( value, row, index ){
                            return `<div><a class="btn btn-sm btn-edit" href="{{env('APP_URL')}}/admin/user/edit/${row.id}"><i class="la la-pencil"></i></a>
                            <a class="btn btn-sm btn-danger" href="javascript:void(0)" onclick="removeEvent(${row.id})"><i class="la la-trash-o"></i></a></div>`
                        }
                    }
                ];
            initializeDatatable({
                Url: '{{env('APP_URL')}}/admin/user?request=table',
                Columns: Columns 
            })
        }
        const removeEvent = (d) => {
        commonAjax({
            "page": `admin/user/${d}?_token={{ csrf_token() }}`,
            "type": 'DELETE'
        }).then(response => {

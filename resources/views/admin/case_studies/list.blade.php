        ...
        <div class="row page-titles mx-0">
            ...
            <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="javascript:void(0);">All Case Studies</a></li>
                </ol>
            </div>
        </div>
        <!-- Content Body -->
        ...
        window.onload = () => {
            const Columns = [
                    ...
                    },
                    {
                        ...
                        title: 'SL. NO.',
                        formatter : function ( value, row, index ){
                            return `<img src="{{ asset('storage/uploads/case_studies/OverviewImage${row.id}.jpg') }}"class="img-fluid" style="width:25px;">`;
                        }
                    }, 
                    {
                        field: 'title',
                    ...
                    },
                    {
                        ...
                        clickToSelect: false,
                        formatter : function ( value, row, index ){
                            return `<div><a class="btn btn-sm btn-edit" href="{{env('APP_URL')}}/admin/case_studies/edit/${row.id}"><i class="la la-pencil"></i></a>
                            <a class="btn btn-sm btn-danger" href="javascript:void(0)" onclick="removeCase(${row.id})"><i class="la la-trash-o"></i></a></div>`
                        }
                    }
                ];
            initializeDatatable({
                Url: '{{env('APP_URL')}}/admin/case_studies?request=table',
                Columns: Columns 
            })
        }
        const removeCase = (d) => {
        commonAjax({
            "page": `admin/case_studies/${d}?_token={{ csrf_token() }}`,
            "type": 'DELETE'
        }).then(response => {

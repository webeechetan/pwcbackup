...
<div class="modal fade" id="messageModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body" data-append="message">
...
@section('script')
    ...
    <script>
        window.onload = () => {
            const Columns = [
                    ...
                    },
                    {
                        ...
                        sortable: true,
                        formatter : function ( value, row, index ){
                            return `<a href="javascript:void(0)" class="text-warning" onclick="viewMessage(${row.id})" data-bs-toggle="modal" data-bs-target="#messageModal">
                                view message
                            </a>`
                        }
                    },
                    {
                        ...
                        clickToSelect: false,
                        formatter : function ( value, row, index ){
                            return `<div><a class="btn btn-sm btn-danger" href="javascript:void(0)" onclick="removeContact(${row.id})"><i class="la la-trash-o"></i></a></div>`
                        }
                    }
                ];
            initializeDatatable({
                Url: '{{env('APP_URL')}}/admin/contact?request=table',
                Columns: Columns 
            })
        }
        const removeContact = (d) => {
            commonAjax({
                "page": `admin/contact/${d}?_token={{ csrf_token() }}`,
                "type": 'DELETE'
            }).then(response => {
        ...
        const viewMessage = (i) => {
            document.querySelector("div[data-append='message']").innerHTML = 'Loading...';
            commonAjax({
                "page": `admin/contact?_token={{ csrf_token() }}&request=message&id=${i}`,
                "type": 'GET'
            }).then(response => {
                let success = response.success || false;
                let element = 'Unable to fetch data!';
                if(success) {
                    element = `<p>${response.data.message}</p>`;
                }
                document.querySelector("div[data-append='message']").innerHTML = element;
            });
        }
    </script>
@endsection